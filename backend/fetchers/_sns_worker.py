...
This breaks the standard shopify.py URL builder, so we handle it here.
Same approach as _end_worker.py — isolate store-specific logic.
...
4. avadaVariants JS object — variant data with barcodes

Sizes are in US format (window.SNS.sizeType = "us") — caller must
convert to EU using convert_to_eu().
"""
import re
import json
import time
...
def _extract_handle_from_url(product_url: str) -> str:
    ...
    parts = path.split("/")
    # Find 'products' segment and take the next part
...
def _fetch_json_endpoint(handle: str) -> dict:
    ...
    resp = request_with_retry(url, session=SESSION, timeout=15)
    ...
    try:
        ...
        for block in ld_blocks:
            try:
                ...
                # Look for ProductGroup or Product type
...
def fetch_sns_page(product_url: str) -> dict:
    ...
    Returns dict with keys:
        json_data: from .json endpoint (Shopify product object)
        js_data:   from .js endpoint (availability, may be None)
        ld_data:   from ld+json in HTML (GTINs/EANs, may be None)
...
def check_sns_still_online(product_url: str) -> dict:
    ...
    Uses the .js endpoint with locale prefix for real-time stock.
    """
    handle = _extract_handle_from_url(product_url)
    url = f"{SNS_BASE}{SNS_LOCALE}/products/{handle}.js"

    try:
        resp = request_with_retry(url, session=SESSION, timeout=10)
